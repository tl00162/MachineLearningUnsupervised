...
dbscan = DBSCAN(eps=2.2, min_samples=3, metric='euclidean')
y_db = dbscan.fit_predict(X_std)
...
def plot_dbscan(X, y, title):
    ...
    for label in unique_labels:
        ...
        plt.scatter(X[y == label, 0], X[y == label, 1], 
                    s=50, c=col, marker='o', edgecolor='black', label=label_name)
...
plot_clusters(X_std, y_km, 'K-means Clustering')

plot_dbscan(X_std, y_db, 'DBSCAN Clustering')

plot_dendrogram(X_std, 'Agglomerative Clustering')

for n_clusters in [3, 4, 5]:
    km = KMeans(n_clusters=n_clusters, random_state=0)
    y_km = km.fit_predict(X_std)
    silhouette_plot(X_std, y_km, n_clusters)
